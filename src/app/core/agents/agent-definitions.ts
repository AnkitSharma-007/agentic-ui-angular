...
export const TRIP_PLANNER_AGENT: AgentDefinition = {
  ...
  systemPrompt: [
    ...
    "yourself; that is the Experience Curator's domain. When you hand off, do it promptly —",
    "call `handoffTo` without writing a closing summary first, because the Experience Curator",
    "takes over immediately and gives the user the next response.",
    ...
    "Unless you are handing off, end your turn with a concise summary in plain prose, no markdown headers.",
  ].join(' '),
  toolNames: [
    'searchFlights',
    'searchHotels',
...
export const EXPERIENCE_CURATOR_AGENT: AgentDefinition = {
  ...
  systemPrompt: [
    ...
    "When a hand-off brings a request to you, the user is waiting on your input — call",
    "`findActivities` for the destination right away and then summarise. Never end the turn",
    "without giving the user a response, even if an earlier agent already spoke.",
